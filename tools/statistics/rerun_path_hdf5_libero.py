...
parser.add_argument('--data-dir', type=str, 
                    default='/ssd/libero_100/libero_90/',
...
parser.add_argument('--save-matplotlib', type=str, default='libero_action_space.png', help='Save a static matplotlib visualization to a PNG file')
    ...
    try:
        with h5py.File(file_path, 'r') as f:
            ...
            if 'data' in f :
                all_pos, all_quat = [], []
                for i in range(10): 
                    if f'data/demo_{i}/obs/ee_pos' not in f :
                        return np.array([]), np.array([])
                    #
                    # Extract TCP pose (end-effector pose) as numpy array
                    tcp_poses = np.array(f[f'data/demo_{i}/obs/ee_pos'])
...
for i, file_path in enumerate(hdf5_files):
    ...
    positions_list, quats_list = load_trajectory_data(file_path)
    # print(positions_list)
    for positions, quats in zip(positions_list, quats_list):
        ...
        if isinstance(positions, np.ndarray) and positions.size > 0:
            ...
            total_cnt += 1 
            visualize_trajectory(total_cnt, filtered_positions, filtered_quats, color, task_name, file_name, args.position_only)
...
def save_matplotlib_visualization(trajectories, task_colors, output_file='trajectory_visualization.png'):
    ...
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    # Use try-except for 3D specific methods that may cause linting errors
    try:
        ax.set_zlabel('Z')
    except AttributeError:
        print("Warning: Could not set Z-axis label")
    ...
    try:
        ...
        ax.scatter(X.flatten(), Y.flatten(), Z.flatten(), color='gray', alpha=0.2, s=1)
