...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Test VLA/HVLA on a primitive tabletop task")
    ...
    parser.add_argument("--external-camera", type=str, default="left_camera",
                        choices=["human_camera", "base_front_camera",
    ...
    parser.add_argument("--cameras", type=str, nargs='+', default=["human_camera", "hand_camera", "base_front_camera"],
                        choices=["base_front_camera", "left_camera",
                                 "right_camera", "human_camera", "hand_camera"],
                        help="Camera to use for external viewing")
...
def run_single_task(args, env_id):
    ...
    if args.hierarchical:
        ...
        if "gpt" in args.llm_model.lower() or "text-davinci" in args.llm_model.lower() or "o" in args.llm_model.lower():
            ...
            llm_provider = "openai"
            logger.warning(
                f"Could not determine provider for model {args.llm_model}, defaulting to OpenAI")

        # Add the detected provider to args
        args.llm_provider = llm_provider
        logger.info(
            f"Creating hierarchical agent with {args.llm_model} from {llm_provider} and {args.vla_agent} VLA agent")
    ...
    for episode in range(args.num_episodes):
        ...
        while not (done or truncated) and step < args.max_steps:
            ...
            if agent:
                try:
                    if args.hierarchical:
                        ...
                        action = agent.get_action(obs, info['description'], step, args.external_camera, args.workflow, base_dirs)
                    else:
                    ...
                    # if args.hierarchical:
                    #     # For hierarchical agent
                    #     action = agent.get_action(obs, "open the cabinet door", step, args.external_camera, args.workflow, base_dirs)
                    # else:
                    #     # For regular VLA agent
                    #     action = agent.get_action(obs, "open the cabinet door", step, args.external_camera)

                    # If action is None despite no exception, still treat it as a failure
                    if action is None:
                        logger.error(
                            "Agent returned None action. Stopping episode to prevent contaminated results.")
                        logger.info(
                            "Episode marked as failed due to LLM/VLA agent error.")
                        env.set_episode_metadata(
                            "failure_reason", "agent_returned_none")
                        break
                except Exception as e:
                    logger.error(f"Error getting action from agent: {e}")
                    logger.error(traceback.format_exc())
                    logger.info(
                        "Episode marked as failed due to LLM/VLA agent exception.")
                    env.set_episode_metadata(
                        "failure_reason", "agent_exception")
                    break
            else:
                logger.error(
                    "Agent is None. Stopping episode to prevent contaminated results.")
                break
            ...
            # Print progress
            logger.info(
                f"Step {step}, Reward: {reward_value:.4f}, Success: {success}")
        ...
        logger.info(
            f"Done: {done}, Truncated: {truncated}, Success: {info.get('success', False)}")
...
def test_all_envs(args):
    ...
    if args.interactive:
        ...
        # # pi0-gemini
        # start = 'Tabletop-Open-Cabinet-WithObstacle-v1'
        # idx = name_list.index(start)
        ...
        # # pi0-gemini-reverse
        # name_list = name_list[::-1]
        # start = 'Tabletop-Put-Cube-IntoCabinetWithObstacle-v1'
        # idx = name_list.index(start)
        # name_list = name_list[idx:]
        # print(name_list)
        # print(f"num of envs={len(name_list)}")
        ...
        # # pi0-gpt4o
        # start = 'Tabletop-Open-Cabinet-WithSwitch-v1'
        # idx = name_list.index(start)
        # name_list = name_list[idx+1:]
        # print(name_list)
        # print(f"num of envs={len(name_list)}")
        # input("Press Enter to continue...")
        ...
        # # pi0-gpt4o-reverse
        # name_list = name_list[::-1]
        # start = 'Tabletop-Seek-Holder-InCabinet-v1'
        # idx = name_list.index(start)
        # name_list = name_list[idx+1:]
        # print(name_list)
        # print(f"num of envs={len(name_list)}")
        ...
        # # gr00t-gpt4o
        # start = 'Tabletop-Open-Door-WithObstacle-v1'
        # idx = name_list.index(start)
        # name_list = name_list[idx+1:]
        # print(name_list)
        # print(f"num of envs={len(name_list)}")
        ...
        # name_list = name_list[::-1]
        # start = 'Tabletop-Pick-Cube-Slippery-v1'
        # index = name_list.index(start)
        # name_list = name_list[index+1:]

        # # cogact-gemini
        # name_list = ["Tabletop-Open-Cabinet-WithObstacle-v1"]
        # print(name_list)
        # print(f"num of envs={len(name_list)}")
        # input("Press Enter to continue...")

        # # gr00t workflow
        # start = "Tabletop-Pick-Cube-WithStick-v1"
        # index = name_list.index(start)
        # name_list = name_list[index+1:]
        # print(name_list)
        # print(f"num of envs={len(name_list)}")
        ...
        # # cogact workflow
        # start = "Tabletop-Pick-Cylinder-WithObstacle-v1"
        # index = name_list.index(start)
    ...
    # name_list = ["Tabletop-Open-Cabinet-WithSwitch-v1"]
    # print(name_list)
    # print(f"num of envs={len(name_list)}")
    # input("Press Enter to continue...")
