...
def parse_args():
    ...
    parser.add_argument("-e", "--env-id", type=str, required=True,
                       help="Environment ID (e.g., Tabletop-Close-Door-v1)")
    ...
    parser.add_argument("--observation-frequency", type=int, default=10,
    ...
    parser.add_argument("--external-camera", type=str, default="left_camera",
                       choices=["base_camera", "base_front_camera", "left_camera", "right_camera"],
...
def main():
    ...
    # Log run information
    logger.info(f"Starting run for {args.env_id} with {args.vla_agent} agent")
    ...
    for episode in range(args.num_episodes):
        logger.info(f"\n{'='*30} Starting Episode {episode+1}/{args.num_episodes} {'='*30}")
        
        # Reset environment
        try:
            obs, info = env.reset(seed=args.seed + episode)
            logger.info(f"Environment reset: {args.env_id}")
        ...
        while not (done or truncated) and step < args.max_steps:
            ...
            if args.show_cameras:
                try:
                    ...
                    camera_views = {}
                    camera_views['human_camera'] = obs['sensor_data']['human_camera']['rgb'][0]
