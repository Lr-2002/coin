...
@register_env("Tabletop-Find-Book-Black-v1", max_episode_steps=5000)
class FindBookWithWordEnv(UniversalTabletopEnv):
    description = (
        "Find and pick the black book from the bookshelf and put it on the marker "
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.tags = {
            ...
            "obj": [Obj.GEOMETRY, Obj.OBSTACLE, Obj.MESH],
            "iter": [Inter.HISTORY]
        }
        self.book_configs = [
                    "configs/book1.json",
        ...
        self.query_query = "Where is the black book?"
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        if self.bookshelf is not None:
            self.bookshelf.set_pose(
                sapien.Pose(
                    p=[0.25, 0.0, self.table_height + 0.3],  # Center of the table
                    q=euler2quat(
                        *np.deg2rad([90, 0, -90])
        ...
        for i, book in enumerate(self.books):
            ...
            position_x = 0.0 + 0.05 * (i / 3)
            position_y = -0.4 + 0.3 * (
                i % 3
            )  # Place books toward the front of the shelf
