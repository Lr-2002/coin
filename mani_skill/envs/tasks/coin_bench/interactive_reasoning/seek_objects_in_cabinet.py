...
@register_env("Tabletop-Seek-Objects-InCabinet-v1", max_episode_steps=5000)
class SeekObjectsInCabinetEnv(UniversalTabletopEnv):
    description = "Find the apple and the plate in the cabinet, put them on the table"
    workflow = [
        "open the drawer",
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.tags = {
            ...
            "iter": [Inter.FAIL_ADAPT, Inter.PLAN, Inter.HISTORY],
        }
        self.object_friction = random.uniform(0.1, 1)
        super().__init__(*args, **kwargs)
        self.query_query = "Where are the apple and plate?"
        ...
        # Did it pick up the apple or plate?
        # Did it put the apple or plate to the table?
    ...
    def _load_scene(self, options: dict):
        ...
        self.plate = self.load_from_config(
            "configs/plate_new.json", "plate", convex=False
        )

    def _load_apple(self):
        """Load an apple from config file"""
        apple_config = "configs/apple.json"
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        self.set_articulation_joint(self.cabinet, "joint_0", 0.1, easy_pd=(1, 0.1))
        self.set_articulation_joint(self.cabinet, "joint_1", 0.1, easy_pd=(1, 0.1))
        cabinet_position = self.cabinet.pose.p[0]
        # cabinet_position[0] = 0
        # cabinet_position[1] = 0
        ...
        self.plate.set_pose(
            sapien.Pose(
                p=cabinet_position + np.array([0, 0, 0.4]),
        ...
        # self.set_articulation_joint(self.cabinet, 'joint_1', 1, pd_version='hard')
    ...
    def _get_success(self, env_idx=None):
        ...
        if (
            self.calculate_object_distance(self.apple, self.goal) <= 0.04
            and self.calculate_object_distance(self.apple, self.plate) <= 0.07
        ):
