...
class CloseCabinetWithObstacleEnv(UniversalTabletopEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    workflow = ["open the door", "move out the obstacle", "close the door"]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.query_query = (
            "Which approach would most likely succeed in closing the cabinet?"
        )
    ...
    def _load_scene(self, options=None):
        ...
        self.block = actors.build_box(
            self.scene, [0.2, 0.02, 0.3], np.array([1, 1, 1, 1]), name="obstacle"
        )
    ...
    def _initialize_episode(self, env_idx, options=None):
        ...
        self.block.set_pose(sapien.Pose(p=[-0.2, -0.28, 0]))
        self.block.set_mass(0.1)
    ...
    def _get_success(self, env_idx=None):
        ...
        if (
            self.get_articulation_joint_info(self.cabinet, self.target_joint_name)
            <= 0.1
        ):
