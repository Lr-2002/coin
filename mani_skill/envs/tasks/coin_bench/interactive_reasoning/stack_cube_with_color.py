...
@register_env("Tabletop-Stack-Cube-WithColor-v1", max_episode_steps=5000)
class StackCubeWithColorEnv(UniversalTabletopEnv):
    """
    ...
    description = "Stack the cube with same color"
    workflow = [
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.query_query = "How many degrees should A be rotated along y axis(the axis vertical to the white face) to make it same color with B?"
        self.query_selection = {"A": "90", "B": "180", "C": "270"}
        self.query_answer = "A"
        # Did it rotate the cube
        # Did it rotate the cube twice?
        # Did it pick up the correct cube?
    ...
    def _load_scene(self, options: dict):
        ...
        #     "configs/rubik_cube.json", "swcw", [-0.8, 0.1, 0.08]
        # )
    ...
    def _check_color(self):
        cp1 = self.compare_obj_angle(self.seal1, self.seal2, threshold=15)
        # cp2 = self.compare_obj_angle(self.seal3, self.seal2)
        # cp3 = self.compare_obj_angle(self.seal3, self.seal1)
        # print(cp1, cp2, cp3)
        return cp1

    def _check_stack(self):
        max_height = max(
            [self.get_aabb(cube)[1][2] for cube in [self.seal1, self.seal2]]
        )
        return max_height

    def _check_stable(self):
        stable = min([self.is_stable(cube) for cube in [self.seal1, self.seal2]])
        return stable
    ...
    def _get_success(self, env_idx=None):
        ...
        print(self._check_stack() >= 0.1, self._check_color(), self._check_stable())
        if self._check_stack() >= 0.1 and self._check_color() and self._check_stable():
