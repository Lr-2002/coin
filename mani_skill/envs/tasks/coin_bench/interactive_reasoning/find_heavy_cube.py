...
@register_env("Tabletop-Find-Cube-WithPivot-v1", max_episode_steps=5000)
class FindHeavyCubeEnv(UniversalTabletopEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    description = "Move the heavy cube to the goal region"
    ...
    def _load_scene(self, options: dict):
        ...
        self.goal = self._create_goal_area(position=[-0.1, 0.2, 0])
    ...
    def _get_success(self, env_idx=None):
        ...
        if self.calculate_object_distance(
            self.cube2, self.goal
        ) <= 0.04 and self.is_stable(self.cube2):
