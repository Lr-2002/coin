...
@register_env("Tabletop-Move-Cross-WithStick-v1", max_episode_steps=5000)
class MoveCrossWithStickEnv(UniversalTabletopEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    description = "Use the stick to move the small cube along the cross-shaped path to the target position "
    workflow = [
        "pick the stick",
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.query_query = (
            "How should the robot navigate the cube through the T-junction?"
        )
    ...
    def _create_t_path(self):
        ...
        left_h_wall_1 = self._create_cuboid(
            name="left_h_wall_1",
        ...
        self.path_cubes["left_h_wall_1"] = left_h_wall_1
        left_h_wall_2 = self._create_cuboid(
            name="left_h_wall_2",
            half_size=[horizontal_length / 6, wall_thickness / 2, wall_height / 2],
            mass=2.0,
            color=[0.5, 0.5, 0.5, 1.0],  # Gray
        ...
        self.path_cubes["left_h_wall_2"] = left_h_wall_2
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        if "left_h_wall_1" in self.path_cubes:
            self.path_cubes["left_h_wall_1"].set_pose(
                sapien.Pose(
                    p=[
                        base_pos_x + horizontal_length / 6 * 2,
        ...
        if "left_h_wall_2" in self.path_cubes:
            self.path_cubes["left_h_wall_2"].set_pose(
                sapien.Pose(
                    p=[
                        base_pos_x - horizontal_length / 6 * 2,
                        base_pos_y - slot_width / 2 - wall_thickness / 2,
