...
class RewardPlotter:
    ...
    def __init__(
        ...
        initial_frame_size: Tuple[int, int] = (512, 512)
    ):
        """
        ...
        Args:
            ...
            initial_frame_size: Size of the initial frame to render on (width, height)
        """
        ...
        # Create an initial blank frame and render on it
        self.current_frame = np.zeros((initial_frame_size[1], initial_frame_size[0], 3), dtype=np.uint8)
        self.current_frame = self.render(self.current_frame)
        
        # Display the initial frame
        cv2.imshow("Reward Plot", self.current_frame)
        cv2.waitKey(1)
    ...
    def update(self, reward: Union[float, np.ndarray, List[float]]) -> np.ndarray:
        """
        Update the reward plot with a new reward value and automatically render.
        
        Args:
            reward: The reward value to add to the plot
            
        Returns:
            The updated frame with the plot overlay
        """
        ...
        # Automatically render and display the updated plot
        self.current_frame = self.render(self.current_frame)
        cv2.imshow("Reward Plot", self.current_frame)
        cv2.waitKey(1)
